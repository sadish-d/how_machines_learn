...
"""
function line_fit(slope::Real, x::Vector, y::Vector)
	return slope .* (x .- mean(x)) .+ mean(y)
end
"""
L2 Loss
"""
function sq_loss(prediction::Vector, actual::Vector)
	return sum((prediction .- actual).^2) # The square root of this is the L2 norm. But argmin of L2 norm is the same as the argmin of (L2 norm)².
end
"""
numerical differentiation
"""
function num_diff(f::Function, x::Real, ϵ::Real)
	return (f(x+ϵ) - f(x)) / ϵ
end
"""
gradient descent
"""
function grad_descent(loss_function, init=0; trials=21)
	b = init
	bs = []
	losses = []
	gradients = []
	learning_rate = 0.014
	# learning_rate = 0.1 # anything between 0.04 to 0.1 seems to land exactly on zero within 30 trials and break the loop below. Also, this really breaks PlutoUI slider and Plot combination.
	for i in 1:trials
		gradient = num_diff(loss_function, b, 1e-6)
		push!(bs, b)
		push!(losses, loss_function(b))
		push!(gradients, gradient)
		b = b - learning_rate * gradient
		# gradient==0 ? break : b = b - learning_rate * gradient
	end
	return (bs, losses, gradients)
end
md"""
functions and other code parameters are defined here
"""
end
...
# ╔═╡ 78631f62-5267-42a4-ae41-52895d116408
begin
scatter(
	pl.lw,
	pl.pr,
	...
	title="refugee flows and social ties",
	ylabel="refugees per 1,000 people",
	xlabel="log₂ of social connectedness index"
)
end

# ╔═╡ 4209985c-7d27-4267-a2f9-134c82b77646
md"""
Each dot here represents a NUTS[^1] region. On the vertical axis, we have the number of refugees per 1,000 residents. And on the horizontal axis, we have the strength of the social connections. An increase of 1 in the horizontal axis means a doubling of the strength of social connections.

It looks like the refugees were drawn to places where they knew more people. More refugees per capita went to Polish regions with stronger social connections with Ukraine. But what is the relationship exactly? One way to answer the question is to **find a straight line that best captures the orientation of the yellow dots**. Let's try.
"""

# ╔═╡ 4a4fa1b4-3be4-419d-ad80-9ee0d7f437a7
md"""
...
Knowing what the machine knows, try and guess:

#### How many more refugees per 1,000 residents go to a place whose social connection with Ukraine is twice as strong?

$(@bind b Slider(parameter_range, default=parameter_range[1], show_value=true))
"""
# $(@bind b Slider(parameter_range, default=0, show_value=true)) $(@bind submit Button("try"))

# ╔═╡ 060c69d6-7c54-46e8-8a6f-dcbe1eff9d97
md"""
## What does a machine do?

I've created a machine. Let's have it guess the answer. How many chances should we give it? $(@bind chances Scrubbable(2:30, default=5))

Now slide the slider to see the sequence of the machine's consecutive guesses.

"""

# ╔═╡ 28c21e02-a5d7-41c9-8102-5a205ff5802c
md"""
$(@bind guess Slider(1:chances))
"""

# ╔═╡ 00c1f4ba-edaa-41b7-9008-b4d959e8d4c4
md"""
How the machine improves its guesses is similar to how we would do it. If a guess is too high, we lower our guess the next time. If it is too low, we guess higher next time. We update our guesses with smaller and smaller amounts, and get closer to the answer. The more chances the machine has, the closer it gets to the answer. Just like us.
"""
...
begin
#
line 		= line_fit(b, pl.lw, pl.pr)
loss 		= sq_loss(line, pl.pr)
function loss_function(x)
	return line_fit(x, pl.lw, pl.pr) |> fit->sq_loss(fit, pl.pr)
end
loss_rate 	= num_diff(loss_function, b, 1e-6)
#
maxloss = let
	loss_bottom = loss_function(parameter_range[begin])
	loss_top 	= loss_function(parameter_range[end])
	max(loss_bottom, loss_top)
end
gd = grad_descent(loss_function, trials=chances)
line_gd = line_fit(gd[1][guess], pl.lw, pl.pr)
md"""
machine learning happens here
"""
end

# ╔═╡ 93c0cb46-ef6f-41b2-bc9d-d225ea9d69f1
begin
# what a machine sees ----------------------------------------
loss_plot = scatter(
	[b],
	[loss],
	# xlims=(-1, 1),
	xlims=(parameter_range[begin]-2, parameter_range[end]+2),
	ylims=(-10_000, 1.3*maxloss),
	legend=false,
	# axis=false,
	# grid=false,
	background_color=RGB(0.2, 0.2, 0.2),
	...
	markerstrokewidth=0,
	title="what a machine sees"
	# size=(200, 400)
)
plot!(
	[b, b],
	[loss, loss-10*loss_rate],
	color=RGB(1, 0, 1)
)
# ------------------------------------------------------------
...
if chances<18
	md"""
	The machine's best guess was **$(round(gd[1][end], digits=2))**. Try giving it more guesses.
	"""
else
	md"""
	The machine's best guess was **$(round(gd[1][end], digits=2))**. Was your guess close?
	"""
end
